...
class DNSKeySyncInstance(service.Service):
    ...
    def __setup_softhsm(self):
        ...
        if constants.NAMED_SSL_ENGINE is not None:
            # setup OpenSSL config file
            # this one is needed because FreeIPA installation
            # disables p11-kit-proxy PKCS11 module
            # https://github.com/OpenSC/libp11
            openssl_conf_txt = ("# OpenSSL configuration file\n"
                                "# File generated by IPA instalation\n"
                                ...
                                "[engine_section]\n"
                                "{ssl_engine}={ssl_engine}_section\n\n"
                                "[{ssl_engine}_section]\n"
                                "engine_id={ssl_engine}\n"
                                "MODULE_PATH={softhsm_module}\n"
                                "init=0\n").format(
                                    openssl_conf=paths.OPENSSL_CONF,
                                    ssl_engine=constants.NAMED_SSL_ENGINE,
                                    softhsm_module=paths.LIBSOFTHSM2_SO)
            logger.debug("Creating new OpenSSL config file")
            with open(paths.DNSSEC_OPENSSL_CONF, 'w') as f:
                os.fchmod(f.fileno(), 0o644)
                f.write(openssl_conf_txt)
        ...
        for sysconfig in [paths.SYSCONFIG_NAMED,
            ...
            if constants.NAMED_SSL_ENGINE is not None:
                directivesetter.set_directive(sysconfig, 'OPENSSL_CONF',
                                              paths.DNSSEC_OPENSSL_CONF,
                                              quotes=False, separator='=')
        if constants.NAMED_SSL_ENGINE is not None:
            named_ssl_engine = "-E {}".format(constants.NAMED_SSL_ENGINE)
            directivesetter.set_directive(paths.SYSCONFIG_NAMED,
