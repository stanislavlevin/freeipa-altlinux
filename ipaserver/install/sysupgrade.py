...
def _load_sstore():
    global _sstore
    if _sstore is None:
        _sstore = sysrestore.StateFile(
            paths.SYSUPGRADE_STATEFILE_DIR, paths.SYSUPGRADE_STATEFILE_FILE
        )
...
def remove_upgrade_file():
    try:
        os.remove(
            os.path.join(
                paths.SYSUPGRADE_STATEFILE_DIR,
                paths.SYSUPGRADE_STATEFILE_FILE,
            )
        )
