...
class TestSudo(IntegrationTest):
    ...
    @pytest.mark.skip_if_hostplatform(
        ...
        hostindex=0,
        platform="debian",
        reason="NISDOMAIN has not been set on Debian",
    )
    @pytest.mark.skip_if_hostcontainer(
        "clients",
        hostindex=0,
        container="any",
        reason="NISDOMAIN cannot be set in containerized environment",
    )
    ...
    def test_sudo_rule_restricted_to_one_hostmask_setup(self):
        ...
        self.__class__.client_sssd_conf_backup = FileBackup(
            self.client, self.client.ipaplatform.paths.SSSD_CONF
        )
