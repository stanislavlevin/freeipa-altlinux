...
class TestHttpKdcProxy(IntegrationTest):
    ...
    def client_use_kdcproxy(self, mh):
        ...
        krb5conf_backup = tasks.FileBackup(
            self.client, self.client.ipaplatform.paths.KRB5_CONF
        )
        krb5conf = self.client.get_file_contents(
            self.client.ipaplatform.paths.KRB5_CONF, encoding="utf-8"
        )
        ...
        self.client.put_file_contents(
            self.client.ipaplatform.paths.KRB5_CONF, krb5conf
        )
    ...
    def configure_kdc_proxy_for_ad_trust(self, use_tcp):
        backup = tasks.FileBackup(
            self.master, self.master.ipaplatform.paths.KDCPROXY_CONFIG
        )
